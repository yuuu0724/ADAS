...
void run_inference(void *arg)
{
    ...
    for (int i = 0; i < od_results.count; i++)
    {
        ...
        if (strcmp(cls_name, "nosafebelt") == 0)
        {
            auto now = steady_clock::now();
            if (duration_cast<seconds>(now - last_alarm_time).count() > 5)
            {
                last_alarm_time = now;
                enqueue_audio("nosafebelt");  // 发送消息队列
                cout << "[ALARM] 未系安全带！" << endl;
            }
        }
    }
...
int main(int argc, char const *argv[])
{
    ...
    for (int i = 0; i < NPU_CORE_NUM; i++)
        if (init_yolov5_model("../model/seatbelt_best.rknn", &yolov5_ctx[i]) != 0) {
